...
DROP PROCEDURE IF EXISTS p_dm_reject_reason_by_apply_org//

CREATE PROCEDURE p_dm_reject_reason_by_apply_org()
BEGIN
    -- 创建表(如果不存在)
    CREATE TABLE IF NOT EXISTS dm_reject_by_org (
    ...
    -- 清空目标表
    TRUNCATE TABLE dm_reject_by_org;

    -- 插入并转换数据
    INSERT INTO dm_reject_by_org (
        apply_org,
        org_code,
        org_name,
        work_group,
