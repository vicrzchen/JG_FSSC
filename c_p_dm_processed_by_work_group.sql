...
DROP PROCEDURE IF EXISTS p_dm_processed_by_work_group//

CREATE PROCEDURE p_dm_processed_by_work_group()
BEGIN
    -- 创建表(如果不存在)
    CREATE TABLE IF NOT EXISTS dm_processed_tickets (
    ...
    -- 清空目标表
    TRUNCATE TABLE dm_processed_tickets;

    -- 插入并转换数据
    INSERT INTO dm_processed_tickets (
        work_group, 
        processed_count, 
        stat_date,
        stat_year,
