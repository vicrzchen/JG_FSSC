...
FROM (
    ...
    FROM dm_received_tickets
    WHERE (
        (period_type = 'day' AND stat_date = CURDATE()) OR
        (period_type = 'month' AND stat_year = YEAR(CURDATE()) AND stat_month = MONTH(CURDATE())) OR
        (period_type = 'year' AND stat_year = YEAR(CURDATE()))
    )
